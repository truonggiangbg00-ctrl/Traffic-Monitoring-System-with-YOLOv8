"""
roi_drawer.py: Interactive ROI drawing tool with Dynamic Lane Restrictions
"""
...
class ROIDrawer:
    def __init__(self, video_source):
        ...
        self.all_polygons = {}
        self.all_restrictions = {} # [MỚI]: Bộ nhớ lưu luật lệ cho từng làn
        self.lane_counter = 1
        self.mouse_pos = (0, 0)
    ...
    def _get_lane_restrictions(self, lane_name):
        ...
        # [SỬA LỖI XUNG ĐỘT BỘ NHỚ]: Dùng Toplevel thay vì Tk() để gắn ké vào GUI chính
        try:
            root = tk.Toplevel() 
        except tk.TclError:
            root = tk.Tk()
        ...
        for cls in self.available_classes:
            # Gắn chặt biến BooleanVar vào đúng cửa sổ root để không bị mất dữ liệu
        ...
        # Bắt hệ thống chờ người dùng bấm Lưu mới được đi tiếp
        root.wait_window()
    ...
    def run(self) -> tuple:
            ...
            while True:
                ...
                if key in (ord('n'), ord('N')):
                    if len(self.current_polygon) >= 3:
                        ...
                        # [MỚI]: Mở Popup chặn màn hình để bắt người dùng chọn luật
        ...
        # [MỚI]: Trả về CẢ TỌA ĐỘ VÀ LUẬT LỆ
